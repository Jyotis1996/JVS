...
<title>UserDetailsA</title>

<META NAME="Description" CONTENT="UserDetailsA">
...
<body>
	...
	<FORM METHOD="POST"
		ACTION="${pageContext.request.contextPath}/UserDetailsA">
		...
		<TABLE align="center" border="2" cellpadding="2">
			...
			<TR>
				...
				<TD colspan="2"><SELECT name="day"  >
						...
						<OPTION value= ${month } <c:if test="${userBean.birthMonth == 'Month'}">selected</c:if>>Month</OPTION>
						...
						<OPTION value= ${year } <c:if test="${userBean.birthYear == 'Year'}">selected</c:if>>Year</OPTION>
			...
			<TR>
				<TD colspan="3" align="center"><Input class="buttons"
					type="submit" value="Submit" id=submit onclick = "submit();"> <Input class="buttons"
	...
	<script type="text/javascript">
		...
		function submit()
		{
			var Edit_Update = document.forms[0].getElementById("submit").value;

			if(Edit_Update == "Submit")
			{	valid = true;
				...
				if(document.forms[0].day.value == "Day" || document.forms[0].month.value == "Month" || document.forms[0].year.value == "Year" )
					{
					   ...
					   out.println("<TD><font color=red>Please Select valid birthdate</font></TD>");
					   out.println("</TABLE>");
					   valid = false;
					}
				return valid;
				document.forms[0].action = '${pageContext.request.contextPath}/UserDetailsB';
			}
			document.forms[0].submit;
		}
		function clearFormData() {
			document.forms[0].action = '${pageContext.request.contextPath}/UserDetailsA_pre';
			document.forms[0].submit;
		}
