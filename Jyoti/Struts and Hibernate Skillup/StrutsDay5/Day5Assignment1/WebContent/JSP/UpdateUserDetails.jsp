	...
	function setData() {
		<%ArrayList<UserListBean> objArrayList = new ArrayList<UserListBean>();
			...
			<%}
		        ...
		        document.getElementById('bdate').value ='<%=objListBean.getBirthDate()%>';
		        document.getElementById('bmonth').value = Number(<%=month%>).toString();
		        document.getElementById('byear').value = '<%=objListBean.getBirthYear()%>';
...
<div style="padding-top: 50px; padding-left: 400px; float: right">
	<a href="http://localhost:8080/Day5Assignment1/userList.do"><bean:message
			key="link.display.back" /></a> &nbsp;&nbsp;&nbsp;&nbsp; <a
		href="javascript:logout()"><bean:message key="link.display.logout" /></a>
</div>
<body onload="setData();">
	...
	<s:form action="/updateDetails" onsubmit="return checkEmpty()"
		...
		<table class="table1">
			...
			<tr>
				...
				<td><s:text property="firstName" maxlength="49"
						styleClass="size"
						value="<%=objListBean.getFirstName().toString()%>"></s:text></td>
			...
			<tr>
				<td colspan="2"><s:multibox property="areaOfInterest"
						value="Web Programming" styleId="checkweb" /> <bean:message
						key="label.update.webprograming" /></td>
			</tr>
			<tr>
				<td colspan="2"><s:multibox property="areaOfInterest"
						value="Database Programming" styleId="checkdatabase" /> <bean:message
						key="label.update.databaseprograming" /></td>
			</tr>
			<tr>
				<td colspan="2"><s:multibox property="areaOfInterest"
						value="Swing Programming" styleId="checkswing" /> <bean:message
						key="label.update.swingprograming" /></td>
			</tr>
			<tr>
				<td colspan="2"><s:multibox property="areaOfInterest"
						value="Mobile Programming" styleId="checkmob" /> <bean:message
						key="label.update.mobileprograming" /></td>
			</tr>
			...
			<tr>
				<td><s:submit styleClass="button1">
						<bean:message key="button.search.update" />
					</s:submit></td>
				<td><s:reset styleClass="button1 button2"
						onclick="http://localhost:8080/Day5Assignment1/updateUserDetails.do">
